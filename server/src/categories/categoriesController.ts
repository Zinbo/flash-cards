...
router.post('/:categoryId/cards/:cardId/wrongAnswer', async (req: Request, res: Response) => {
  const categoryId = req.params.categoryId
  const cardId = req.params.cardId
...
router.post('/:categoryId/cards/', async (req: Request, res: Response) => {
  ...
  console.log(`Found category: ${category.constructor.name}`)
  const newCard = category.addCard(card.front, card.back)
  categoriesRepository.updateCategory(category)
  res.send(newCard)
